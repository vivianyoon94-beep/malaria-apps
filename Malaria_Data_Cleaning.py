...
# ==========================
# Streamlit File Upload + UI (Dropdown version)


st.set_page_config(page_title="🦟 Malaria Data Cleaning App", layout="wide")
st.title("🦟 Malaria Data Cleaning App")
...
def build_error_summary_by_column(error_series: pd.Series) -> pd.DataFrame:
    counts = {}
...
def force_screening_date_strings(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure SCREENING_DATE is saved as 'DD-MMM-YY' (e.g., '02-May-25').
    - Formats real datetimes and ISO-like strings.
    - Leaves other strings (e.g., '08.4.25') as-is so your COMMENT logic can flag them.
    """
    ...
    def _fmt(v):
        ...
        # ISO-like string -> safe to parse and format
        if isinstance(v, str) and _ISO_DATE.match(v.strip()):
            dt = pd.to_datetime(v, errors='coerce')
            if pd.notna(dt):
                return dt.strftime('%d-%b-%y')
        # anything else: keep original (e.g., '08.4.25')
        return v
...
def multiselect_dropdown(label: str, options: list[str], key: str = "ms_dropdown"):
    """
    Chip-free multi-select using a popover + checkboxes.
    Starts with NO items selected. Resets selection when options change.
    """
    sel_key  = f"{key}_selected"
    snap_key = f"{key}_options_snapshot"

    if sel_key not in st.session_state:
        st.session_state[sel_key] = set()          # start empty
    ...
    if st.session_state[snap_key] != snapshot:
        st.session_state[snap_key] = snapshot      # options changed → clear selection
    ...
    # Use popover if available; fallback to expander on older Streamlit
...
if uploaded_file is not None:
    try:
        ...
        # Chip-free multi-select dropdown (no red chips)
        selected_sheets = multiselect_dropdown("Select sheet(s) to clean", sheet_names, key="sheets")

        if not selected_sheets:
            st.info("Select at least one sheet to continue.")
            st.stop()

        # Clean and collect outputs
        cleaned_by_sheet = {}
        for sheet in selected_sheets:
            raw_df  = xls.parse(sheet_name=sheet)
            cleaned = clean_malaria_data(raw_df)

            # ensure final output like '02-May-25'
        ...
        for tab, sheet in zip(tabs, selected_sheets):
            with tab:
                ...
                if error_df.empty:
                    ...
                    summary_df = build_error_summary_by_column(error_df["COMMENT"])
                    if not summary_df.empty:
                        st.markdown("**📊 Error Summary by Column**")
                        st.dataframe(summary_df, use_container_width=True)

        # Download: single workbook containing all selected sheets (COMMENT last in each)

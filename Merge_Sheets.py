...
def _validate_headers_match(dfs: List[pd.DataFrame]) -> None:
    ...
    for i, d in enumerate(dfs[1:], start=2):
        if set(_normalize_headers(d)) != base:
            raise ValueError(f"Headers do not match across all selected sheets (mismatch near input #{i}).")
...
def write_zip_injected(file_sheet_map: Dict[str, Iterable[str]], merged_df: pd.DataFrame, zip_path: str) -> None:
    """Create a ZIP where each original .xlsx gets a merged sheet appended preserving formatting.
       For .xls inputs, we include a new .xlsx containing only the merged sheet.
    """
    if not _HAVE_OPENPYXL:
        raise RuntimeError("openpyxl not available; cannot preserve formatting for .xlsx. Add 'openpyxl' to requirements.")
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for src_path in file_sheet_map.keys():
            ...
            if fname.lower().endswith(".xlsx"):
                ...
                # .xls fallback: can't preserve formatting, provide merged-only xlsx
...
def parse_select_args(select_args: List[str], files: List[str]) -> Dict[str, List[str]]:
    ...
    if not select_args:
        # default to all sheets for each file
        for f in files:
    ...
    # ensure all files present (if a file missing in --select, default to all sheets)
    for f in files:
        if f in wanted:
            mapping[f] = wanted[f]
        else:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--zip", dest="zip_path", help="If provided, also write a ZIP that injects the merged sheet back into each original file")
    ap.add_argument("--select", action="append", help="Select sheets per file: 'file.xlsx:Sheet1,Sheet2'. Can be specified multiple times.")
    args = ap.parse_args(argv)

    # Build file->sheets mapping
    ...
    # Optional ZIP with injected merged sheets
    if args.zip_path:
